...
import { predictRisk, predictAnomaly, blockSession } from "../ai/anomaly-predict";
...
class PredictBenchmarkRunner {
  ...
  // Run all benchmarks
  async runAllBenchmarks(config: BenchmarkConfig = {
    sessionCount: 1000,
    vectorSize: 5,
    concurrency: 100,
    threshold: 0.92,
    ...
    console.log(`📊 Configuration: ${config.sessionCount} sessions, ${config.concurrency} concurrency`);
    console.log('');
  ...
  private async benchmarkGNNPrediction(
    ...
    console.log(`   ✅ ${vectors.length} predictions in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkVectorValidation(
    ...
    console.log(`   ✅ ${vectors.length} validations in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkFeatureExtraction(
    ...
    console.log(`   ✅ ${vectors.length} extractions in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkInferenceGeneration(
    ...
    console.log(`   ✅ ${vectors.length} inferences in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkBlockEnforcement(
    ...
    for (const vector of vectors) {
      ...
      if (vector.expectedRisk === 'critical' || vector.expectedRisk === 'high') {
    ...
    console.log(`   ✅ ${vectors.length} enforcement checks in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkRiskIndexBuild(
    ...
    console.log(`   ✅ Built risk index with ${riskIndex.length} entries in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkConcurrentSessions(
    ...
    console.log(`   ✅ Processed ${vectors.length} sessions concurrently in ${duration.toFixed(2)}ms`);
  ...
  private async benchmarkMemoryUsage(
    ...
    for (const vector of vectors.slice(0, 100)) { // Sample for memory test
    ...
    console.log(`   📊 Memory per session: ${(memoryPerSession / 1024).toFixed(2)} KB`);
    console.log('');
  }
  ...
  private validateVector(features: number[]): boolean {
    ...
    if (features.length !== requiredLength) {
      throw new Error(`Vector length mismatch: expected ${requiredLength}, got ${features.length}`);
    }
    
    // Check boolean features
    if (![0, 1].includes(features[0])) throw new Error('Invalid root_detected value');
    if (![0, 1].includes(features[1])) throw new Error('Invalid vpn_active value');
    
    // Check ranges
    if (features[2] < 0 || features[2] > 50) throw new Error('Invalid thermal_spike range');
    if (features[3] < 0 || features[3] > 10) throw new Error('Invalid biometric_fail range');
    if (features[4] < 0 || features[4] > 10) throw new Error('Invalid proxy_hop_count range');
  ...
  private generatePerformanceReport(): void {
    ...
    this.results.forEach(result => {
      ...
      console.log(`   Latency: avg=${result.latency.avg.toFixed(2)}ms, p95=${result.latency.p95.toFixed(2)}ms`);
    ...
    const avgImprovement = validImprovements > 0 ? totalImprovement / validImprovements : 0;
    ...
    console.log(`   System Surge: ~${(avgImprovement * this.results.length).toFixed(0)}%`);
    console.log('');
