...
interface Alert {
  ...
  category: 'performance' | 'security' | 'availability' | 'capacity' | 'accuracy';
  title: string;
  message: string;
  timestamp: number;
...
class MonitoringAlertingSystem {
  ...
  private async triggerRule(rule: MonitoringRule, metrics: SystemMetrics, metricValue: number): Promise<void> {
    const alert: Alert = {
      id: `alert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      type: rule.severity >= 8 ? 'critical' : rule.severity >= 6 ? 'warning' : 'info',
      category: this.getAlertCategory(rule.id),
      title: rule.name,
      message: `${rule.description}: Current value ${metricValue.toFixed(2)} ${operator} threshold ${threshold}`,
      timestamp: Date.now(),
  ...
  private getAlertCategory(ruleId: string): Alert['category'] {
    if (ruleId.includes('cpu') || ruleId.includes('memory') || ruleId.includes('latency')) {
      return 'performance';
    }
  ...
  private updateDashboardMetrics(metrics: SystemMetrics): void {
    ...
    console.log(`📊 Metrics Updated - CPU: ${metrics.cpu.usage.toFixed(1)}%, Memory: ${metrics.memory.percentage.toFixed(1)}%, Latency: ${metrics.network.averageLatency.toFixed(1)}ms`);
  }
  ...
  private async createAlert(alertData: Partial<Alert>): Promise<void> {
    const alert: Alert = {
      id: alertData.id || `alert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  ...
  private async sendToChannel(channelId: string, channel: any, alert: Alert): Promise<void> {
    switch (channel.type) {
      case 'console':
        console.log(`📢 [${channelId.toUpperCase()}] ${alert.title}: ${alert.message}`);
        break;
  ...
  public enableAlertChannel(channelId: string, enabled: boolean): boolean {
    ...
    if (channel) {
      ...
      console.log(`✅ Alert channel ${channelId} ${enabled ? 'enabled' : 'disabled'}`);
      return true;
    }
    return false;
  }
...
export { MonitoringAlertingSystem, type Alert, type MonitoringRule, type SystemMetrics, type HealthCheck };
