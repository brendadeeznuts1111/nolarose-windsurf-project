...
import { ExtendedFeatureVector, RiskScoreResult } from "./risk-scoring";
...
export class FraudPatternDetector {
  ...
  async detectPatterns(
    ...
    if (this.config.enableBehavioralAnalysis && userId) {
      const behavioralPatterns = await this.analyzeBehavioralPatterns(
        features, riskResult, sessionId, userId
      );
      detectedPatterns.push(...behavioralPatterns);
    }
    
    // Synthetic identity detection
    const syntheticPatterns = this.detectSyntheticIdentityPatterns(features, riskResult, sessionId);
    detectedPatterns.push(...syntheticPatterns);
    
    // Account takeover detection
    const takeoverPatterns = this.detectAccountTakeoverPatterns(features, riskResult, sessionId);
    detectedPatterns.push(...takeoverPatterns);
    
    // Location anomaly detection
    const locationPatterns = this.detectLocationAnomalies(features, riskResult, sessionId);
    detectedPatterns.push(...locationPatterns);
    
    // Device anomaly detection
    const devicePatterns = this.detectDeviceAnomalies(features, riskResult, sessionId);
    detectedPatterns.push(...devicePatterns);
    
    // Velocity anomaly detection
    const velocityPatterns = this.detectVelocityAnomalies(features, riskResult, sessionId);
    ...
    console.log(`🔍 Pattern Detection: Found ${detectedPatterns.length} patterns for session ${sessionId}`);
  ...
  private async analyzeBehavioralPatterns(
    ...
    if (currentPattern.deviationScore > this.config.anomalyThreshold) {
      patterns.push({
        ...
        recommendedActions: ['REQUIRE_ADDITIONAL_VERIFICATION', 'ENABLE_BEHAVIORAL_MONITORING'],
  ...
  private detectSyntheticIdentityPatterns(
    ...
    if (features.time_since_last_activity < 300) { // 5 minutes
    ...
    if (syntheticScore > 0.7) {
      patterns.push({
        ...
        recommendedActions: ['BLOCK_SESSION', 'TRIGGER_IDENTITY_VERIFICATION', 'ENHANCED_MONITORING'],
  ...
  private detectAccountTakeoverPatterns(
    ...
    if (features.device_age_hours > 8760 && features.time_since_last_activity < 3600) {
    ...
    if (takeoverScore > 0.6) {
      patterns.push({
        ...
        recommendedActions: ['BLOCK_SESSION', 'NOTIFY_ACCOUNT_OWNER', 'FORCE_PASSWORD_RESET'],
  ...
  async analyzeNetwork(sessions: Array<{
    sessionId: string;
    features: ExtendedFeatureVector;
    riskScore: number;
    timestamp: number;
    ...
    featureGroups.forEach((groupSessions, groupKey) => {
      if (groupSessions.length > 2) { // Potential coordinated attack
        const avgRiskScore = groupSessions.reduce((sum, s) => sum + s.riskScore, 0) / groupSessions.length;
        ...
        if (avgRiskScore > 0.7) {
          groupSessions.forEach(session => {
            suspiciousConnections.push({
              ...
              connectedSessions: groupSessions.filter(s => s.sessionId !== session.sessionId).map(s => s.sessionId),
    ...
    const networkRiskScore = suspiciousConnections.length > 0 ? 
      Math.max(...suspiciousConnections.map(c => c.riskScore)) : 0;
    
    const recommendations = networkRiskScore > 0.8 ? [
  ...
  private generateFeatureGroupKey(features: ExtendedFeatureVector): string {
    ...
    if (features.proxy_hop_count > 0) elements.push(`PROXY_${features.proxy_hop_count}`);
  ...
  private identifyClusterPattern(sessions: Array<{
    features: ExtendedFeatureVector;
    riskScore: number;
    timestamp: number;
  }>): FraudPatternType {
    ...
    const isNewDevice = sessions.some((s: any) => s.features.device_age_hours < 24);
    
    if (hasRoot && hasProxy) return FraudPatternType.COORDINATED_ATTACK;
    if (hasVpn && hasProxy && isNewDevice) return FraudPatternType.SYNTHETIC_IDENTITY;
  ...
  clearOldPatterns(olderThanHours: number = 24): void {
    ...
    this.detectedPatterns = this.detectedPatterns.filter(p => p.timeframe.end > cutoff);
  }
