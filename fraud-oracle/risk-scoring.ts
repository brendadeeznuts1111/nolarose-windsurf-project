...
export class FraudRiskOracle {
  ...
  async calculateRiskScore(
    ...
    const recommendations = this.generateRecommendations(finalScore, riskFactors.primary);
    ...
    const result: RiskScoreResult = {
      ...
      requiresAdditionalVerification: finalScore >= RISK_THRESHOLDS.HIGH_RISK && !blocked
    };
    
    console.log(`🎯 Fraud Oracle Score: ${finalScore.toFixed(3)} | ${riskLevel.toUpperCase()} | ${processingTime.toFixed(2)}ms`);
  ...
  private applyTemporalDecay(baseScore: number, features: ExtendedFeatureVector): number {
    let decayFactor = 1.0;
    
    // Device age decay
    if (features.device_age_hours < 24) {
      decayFactor *= (1 + TEMPORAL_DECAY.device_age.new_device);
    } else if (features.device_age_hours < 168) { // 1 week
    ...
    if (features.time_since_last_activity < 3600) { // 1 hour
      decayFactor *= (1 + TEMPORAL_DECAY.account_age.new_account);
    } else if (features.time_since_last_activity < 86400) { // 24 hours
  ...
  private applyGeographicRisk(baseScore: number, features: ExtendedFeatureVector): number {
    let geoRiskMultiplier = 1.0;
    
    // Check for impossible travel velocity
    if (features.location_velocity > GEOGRAPHIC_RISK.impossible_velocity_threshold) {
  ...
  private applyDeviceFingerprintingRisk(baseScore: number, features: ExtendedFeatureVector): number {
    let deviceRiskMultiplier = 1.0;
    
    // Check for emulator signatures
    if (DEVICE_RISK_PATTERNS.emulator_signatures.some(sig => 
      features.device_hash.toLowerCase().includes(sig.toLowerCase()))) {
      deviceRiskMultiplier *= 1.8;
    }
    
    // Check for root indicators
    if (DEVICE_RISK_PATTERNS.root_indicators.some(sig => 
      features.browser_fingerprint.toLowerCase().includes(sig.toLowerCase()))) {
      deviceRiskMultiplier *= 2.0;
    }
    
    // Check for VPN signatures
    if (DEVICE_RISK_PATTERNS.vpn_signatures.some(sig => 
      features.device_hash.toLowerCase().includes(sig.toLowerCase()))) {
      deviceRiskMultiplier *= 1.3;
    }
    
    // Check for suspicious user agents
    if (DEVICE_RISK_PATTERNS.suspicious_user_agents.some(sig => 
      features.browser_fingerprint.toLowerCase().includes(sig.toLowerCase()))) {
  ...
  private calculateConfidence(features: ExtendedFeatureVector, score: number): number {
  ...
  private calculateDataCompleteness(features: ExtendedFeatureVector): number {
    ...
    const nonNullFields = Object.values(features).filter(val => 
      val !== null && val !== undefined && val !== 0).length;
    
    return nonNullFields / totalFields;
  }
  
  // Analyze feature contributions
  private analyzeFeatureContributions(features: ExtendedFeatureVector): Record<string, number> {
    const contributions: Record<string, number> = {};
    const weights = { ...FEATURE_WEIGHTS, ...this.config.customWeights };
    
    // Core 5 features
    contributions.root_detected = features.root_detected * weights.root_detected;
    contributions.vpn_active = features.vpn_active * weights.vpn_active;
    contributions.thermal_spike = features.thermal_spike * weights.thermal_spike;
    contributions.biometric_fail = features.biometric_fail * weights.biometric_fail;
    contributions.proxy_hop_count = features.proxy_hop_count * weights.proxy_hop_count;
    
    // Extended features (lower weight)
    contributions.device_age = Math.min(0.1, features.device_age_hours < 24 ? 0.1 : 0);
    contributions.location_velocity = Math.min(0.1, features.location_velocity > 500 ? 0.1 : 0);
    contributions.failed_logins = Math.min(0.1, features.failed_login_attempts * 0.02);
    
    return contributions;
  }
  
  // Identify primary and secondary risk factors
  private identifyRiskFactors(features: ExtendedFeatureVector, score: number): {
    primary: string[];
    secondary: string[];
  } {
    ...
    if (features.biometric_fail >= 3) primary.push('Multiple biometric failures');
    if (features.thermal_spike > 15) primary.push('Unusual device temperature');
    if (features.location_velocity > 800) primary.push('Impossible travel velocity');
    
    // Secondary risk factors (medium impact)
    if (features.vpn_active) secondary.push('VPN or proxy usage');
    if (features.failed_login_attempts >= 2) secondary.push('Failed login attempts');
    if (features.device_age_hours < 24) secondary.push('New device detected');
    if (features.time_since_last_activity > 86400 * 30) secondary.push('Inactive account');
    if (features.transaction_amount_std > 1000) secondary.push('Unusual transaction pattern');
    
    return { primary, secondary };
  }
  
  // Generate action recommendations
  private generateRecommendations(score: number, primaryRiskFactors: string[]): string[] {
    const recommendations: string[] = [];
    
    if (score >= RISK_THRESHOLDS.CRITICAL_BLOCK) {
      recommendations.push('BLOCK_SESSION', 'TRIGGER_SECURITY_ALERT', 'LOG_FRAUD_ATTEMPT');
    } else if (score >= RISK_THRESHOLDS.HIGH_RISK) {
      recommendations.push('REQUIRE_ADDITIONAL_VERIFICATION', 'ENABLE_STEP_UP_AUTH');
      ...
      recommendations.push('ENABLE_SHADOW_MONITORING', 'LOG_BEHAVIORAL_ANALYTICS');
    } else {
  ...
  getSessionRiskStats(sessionId: string): {
    ...
    const avgScore = history.reduce((sum, score) => sum + score, 0) / history.length;
