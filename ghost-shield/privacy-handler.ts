...
export interface PrivacyServiceFingerprint {
  service: 'privacy_com' | 'duoplus' | 'cloudflare' | 'legitimate_vpn' | 'unknown';
  confidence: number;
  ipRange?: string;
  asn?: string;
  organization?: string;
...
const PRIVACY_SERVICE_PATTERNS = {
  ...
  privacy_com: {
    ip_ranges: [
      '199.102.106.', '199.102.107.', '199.102.108.',
      '199.102.109.', '199.102.110.', '199.102.111.'
    ],
    asn: ['AS40028', 'AS22612'],
  ...
  cloudflare: {
    ip_ranges: [
      '103.21.244.', '103.22.200.', '103.31.244.', '104.16.0.',
      '104.17.0.', '104.18.0.', '104.19.0.', '104.20.0.'
    ],
    asn: ['AS13335'],
  ...
  legitimate_vpn: {
    ip_ranges: [
      ...
      '8.8.8.', '8.8.4.', // Google DNS (often used with VPN)
...
export class GhostShield {
  ...
  private serviceStats: Map<string, { count: number; lastSeen: number }> = new Map();
  ...
  async processGhostShield(
    ...
    const serviceFingerprint = this.detectPrivacyService(
      sessionId, ipAddress, userAgent, organization
    );
    
    // Apply ghost-friendly de-weighting
    const deWeightingResult = this.applyGhostDeWeighting(
      features, serviceFingerprint, sessionId
    );
    
    // Update statistics
    this.updateServiceStats(serviceFingerprint.service);
    
    console.log(`👻 Ghost Shield: ${serviceFingerprint.service} detected | De-weighting: ${deWeightingResult.deWeightingApplied ? 'YES' : 'NO'}`);
  ...
  private detectPrivacyService(
    ...
    if (this.config.enablePrivacyComDeWeighting) {
      const privacyScore = this.checkPrivacyComPatterns(sessionId, ipAddress, userAgent, organization);
      if (privacyScore > confidence) {
        service = 'privacy_com';
        confidence = privacyScore;
        indicators.push(...this.getPrivacyComIndicators(sessionId, ipAddress, userAgent, organization));
      }
    }
    
    // Check DuoPlus patterns
    if (this.config.enableDuoPlusProtection) {
      const duoPlusScore = this.checkDuoPlusPatterns(sessionId, ipAddress, userAgent, organization);
      if (duoPlusScore > confidence) {
        service = 'duoplus';
        confidence = duoPlusScore;
        indicators.length = 0; // Clear previous indicators
        indicators.push(...this.getDuoPlusIndicators(sessionId, ipAddress, userAgent, organization));
      }
    }
    
    // Check Cloudflare patterns
    if (this.config.enableCloudflareWhitelist) {
      const cloudflareScore = this.checkCloudflarePatterns(sessionId, ipAddress, userAgent, organization);
      if (cloudflareScore > confidence) {
        service = 'cloudflare';
        confidence = cloudflareScore;
        indicators.length = 0;
        indicators.push(...this.getCloudflareIndicators(sessionId, ipAddress, userAgent, organization));
      }
    }
    
    // Check legitimate VPN patterns
    if (this.config.enableVPNFriendlyMode) {
      const vpnScore = this.checkLegitimateVPNPatterns(sessionId, ipAddress, userAgent, organization);
      if (vpnScore > confidence) {
        service = 'legitimate_vpn';
        confidence = vpnScore;
        indicators.length = 0;
        indicators.push(...this.getLegitimateVPNIndicators(sessionId, ipAddress, userAgent, organization));
      }
    }
  ...
  private checkPrivacyComPatterns(
    ...
    // Check IP range
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.privacy_com.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      score += 0.4;
    }
    
    // Check organization
    if (organization && PRIVACY_SERVICE_PATTERNS.privacy_com.organization.some(org => 
      organization.toLowerCase().includes(org.toLowerCase()))) {
      score += 0.3;
    }
    
    // Check session patterns
    if (PRIVACY_SERVICE_PATTERNS.privacy_com.session_patterns.some(pattern => 
  ...
  private getPrivacyComIndicators(
    ...
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.privacy_com.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      indicators.push(`Privacy.com IP range: ${this.extractIPRange(ipAddress)}`);
    }
  ...
  private checkDuoPlusPatterns(
    ...
    if (PRIVACY_SERVICE_PATTERNS.duoplus.session_patterns.some(pattern => 
      sessionId.toLowerCase().includes(pattern))) {
      score += 0.5;
    }
    
    // Check AWS ranges (DuoPlus uses AWS)
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.duoplus.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      score += 0.3;
    }
    
    // Check organization
    if (organization && PRIVACY_SERVICE_PATTERNS.duoplus.organization.some(org => 
      organization.toLowerCase().includes(org.toLowerCase()))) {
      score += 0.2;
    }
  ...
  private checkCloudflarePatterns(
    ...
    // Check IP range
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.cloudflare.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      score += 0.5;
    }
    
    // Check organization
    if (organization && PRIVACY_SERVICE_PATTERNS.cloudflare.organization.some(org => 
      organization.toLowerCase().includes(org.toLowerCase()))) {
      score += 0.3;
    }
    
    // Check session patterns
    if (PRIVACY_SERVICE_PATTERNS.cloudflare.session_patterns.some(pattern => 
      sessionId.toLowerCase().includes(pattern))) {
      score += 0.2;
    }
  ...
  private checkLegitimateVPNPatterns(
    ...
    // Check IP range
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.legitimate_vpn.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      score += 0.3;
    }
    
    // Check organization
    if (organization && PRIVACY_SERVICE_PATTERNS.legitimate_vpn.organization.some(org => 
      organization.toLowerCase().includes(org.toLowerCase()))) {
      score += 0.4;
    }
    
    // Check session patterns
    if (PRIVACY_SERVICE_PATTERNS.legitimate_vpn.session_patterns.some(pattern => 
      sessionId.toLowerCase().includes(pattern))) {
      score += 0.3;
    }
  ...
  private getLegitimateVPNIndicators(
    ...
    if (ipAddress && PRIVACY_SERVICE_PATTERNS.legitimate_vpn.ip_ranges.some(range => 
      ipAddress.startsWith(range))) {
      indicators.push(`Legitimate service IP range: ${this.extractIPRange(ipAddress)}`);
    }
  ...
  private applyGhostDeWeighting(
    ...
    switch (serviceFingerprint.service) {
      case 'privacy_com':
        if (this.config.enablePrivacyComDeWeighting) {
          adjustedFeatures.proxy_hop_count = Math.max(0, adjustedFeatures.proxy_hop_count - 2);
          adjustedFeatures.vpn_active = Math.min(1, adjustedFeatures.vpn_active * 0.5);
      ...
      case 'duoplus':
        if (this.config.enableDuoPlusProtection) {
          adjustedFeatures.vpn_active = Math.min(1, adjustedFeatures.vpn_active * 0.6);
          adjustedFeatures.proxy_hop_count = Math.max(0, adjustedFeatures.proxy_hop_count - 1);
      ...
      case 'legitimate_vpn':
        if (this.config.enableVPNFriendlyMode) {
          adjustedFeatures.vpn_active = Math.min(1, adjustedFeatures.vpn_active * 0.7);
  ...
  private updateServiceStats(service: PrivacyServiceFingerprint['service']): void {
...
export type { PrivacyServiceConfig, PrivacyServiceFingerprint, GhostShieldResult };
