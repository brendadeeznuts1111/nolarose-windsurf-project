...
export class ProxyDetector {
  ...
  constructor(config: ProxyDetectionConfig = {
    ...
    maxAllowedHops: 3,
    suspiciousASNs: [
      'AS16276', 'AS13335', 'AS396982', 'AS209242', // Known proxy ASNs
      'AS197695', 'AS20473', 'AS14061', 'AS16509'   // Datacenter ASNs
    ],
    knownProxyRanges: [
      '10.0.0.', '192.168.', '172.16.', // Private ranges
      '100.64.', '198.18.', '169.254.'  // CGNAT and link-local
    ]
  }) {
    this.config = config;
  ...
  async detectProxy(
    ...
    // Traffic pattern analysis
    const trafficAnalysis = this.config.enableTrafficAnalysis && headers
    ...
    const proxyAssessment = this.assessProxyType(
      hopAnalysis, geoAnalysis, trafficAnalysis, dnsAnalysis
    );
    ...
    const recommendations = this.generateProxyRecommendations(
      proxyAssessment, riskScore, hopAnalysis.hopCount
    );
    
    const processingTime = performance.now() - startTime;
    
    console.log(`🔍 Proxy Detection: ${proxyAssessment.type} | Hops: ${hopAnalysis.hopCount} | Risk: ${riskScore.toFixed(3)} | ${processingTime.toFixed(2)}ms`);
    ...
    return {
      ...
      riskScore,
      riskFactors: this.identifyRiskFactors(hopAnalysis, geoAnalysis, trafficAnalysis),
      recommendations,
  ...
  private async analyzeTrafficPattern(headers: Record<string, string>): Promise<TrafficPattern> {
  ...
  private analyzeProtocolDistribution(headers: Record<string, string>): Record<string, number> {
  ...
  private assessProxyType(
    ...
    const knownProxyHops = hopAnalysis.hops.filter(hop => hop.isKnownProxy).length;
  ...
  private generateProxyRecommendations(
    proxyAssessment: { type: ProxyDetectionResult['proxyType']; confidence: number },
    riskScore: number,
    hopCount: number
  ): string[] {
    ...
    if (proxyAssessment.type === 'tor') {
      ...
      recommendations.push('REQUIRE_ADDITIONAL_VERIFICATION', 'DATACENTER_PROXY_DETECTED');
  ...
  private isKnownProxyIP(ip: string): boolean {
    return this.knownProxyIPs.has(ip) || 
  ...
  private calculateHopRiskScore(hopData: any): number {
    ...
    if (hopData.organization?.includes('proxy') || hopData.organization?.includes('vpn')) {
      risk += 0.2;
    }
  ...
  private initializeKnownProxies(): void {
    ...
    console.log(`🔍 Proxy Detector initialized with ${this.suspiciousASNs.size} suspicious ASNs`);
  }
