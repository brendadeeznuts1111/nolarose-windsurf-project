  ...
  Bun.serve({
    ...
    fetch(req) {
      ...
      if (url.pathname.startsWith('/api/family/alerts/') && req.method === 'GET') {
        const guardianId = url.pathname.split('/')[4];
        if (guardianId) {
          return handleGetRiskAlerts(guardianId);
        }
      }
...
// Get Risk Alerts
async function handleGetRiskAlerts(guardianId: string): Promise<Response> {
  try {
...
console.log('🧠 Suspension Risk API Server running on port 3003');
