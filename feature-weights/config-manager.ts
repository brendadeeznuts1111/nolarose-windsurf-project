...
export class FeatureWeightManager {
  ...
  validateWeights(weights: FeatureWeightConfig): WeightValidationResult {
    ...
    const requiredFeatures: (keyof FeatureWeightConfig)[] = [
      'root_detected', 'vpn_active', 'thermal_spike', 'biometric_fail', 'proxy_hop_count'
    ];
    ...
    const totalWeight = requiredFeatures.reduce((sum, feature) => sum + weights[feature], 0);
    if (totalWeight > 15) {
      warnings.push(`Total weight (${totalWeight}) is very high, may cause oversensitivity`);
    } else if (totalWeight < 2) {
      warnings.push(`Total weight (${totalWeight}) is very low, may reduce detection effectiveness`);
    }
    ...
    if (totalWeight > 10) {
      ...
      warnings.push(`Weights normalized by factor ${scaleFactor.toFixed(3)} to prevent oversensitivity`);
    }
  ...
  async optimizeWeights(
    ...
    console.log(`🧠 Optimizing weights based on ${performanceData.length} data points`);
    ...
    const result: WeightOptimizationResult = {
      optimizedWeights,
      improvement,
      confidence: this.calculateOptimizationConfidence(featureImportance, performanceData),
      recommendations: this.generateOptimizationRecommendations(featureImportance, improvement)
    };
    
    console.log(`🎯 Weight optimization complete`);
    console.log(`   Expected accuracy improvement: ${(improvement.accuracy * 100).toFixed(2)}%`);
    console.log(`   Expected FP rate reduction: ${(improvement.falsePositiveRate * 100).toFixed(2)}%`);
  ...
  private calculateFeatureImportance(
    ...
    for (const feature of features) {
      ...
      const contribution = this.calculateFeatureContribution(feature, performanceData);
      
      // Recommend new weight based on importance
      const recommendedWeight = this.calculateRecommendedWeight(correlation, contribution);
  ...
  private calculateFeatureContribution(
    ...
    const avgContribution = featureValues.reduce((sum, value) => 
      sum + (value * currentWeight), 0) / featureValues.length;
    
    return avgContribution;
  }
  
  // Calculate recommended weight based on correlation and contribution
  private calculateRecommendedWeight(correlation: number, contribution: number): number {
  ...
  private generateOptimizedWeights(featureImportance: FeatureImportance[]): FeatureWeightConfig {
  ...
  private calculateExpectedImprovement(
    ...
    const currentAccuracy = this.calculateSimulatedAccuracy(currentWeights, performanceData);
    const optimizedAccuracy = this.calculateSimulatedAccuracy(optimizedWeights, performanceData);
    
    const currentFP = this.calculateSimulatedFalsePositiveRate(currentWeights, performanceData);
    const optimizedFP = this.calculateSimulatedFalsePositiveRate(optimizedWeights, performanceData);
    
    const currentDR = this.calculateSimulatedDetectionRate(currentWeights, performanceData);
    const optimizedDR = this.calculateSimulatedDetectionRate(optimizedWeights, performanceData);
  ...
  private calculateSimulatedAccuracy(
    ...
    for (const data of performanceData) {
      const simulatedScore = this.calculateSimulatedScore(weights, data.features);
  ...
  private calculateSimulatedScore(weights: FeatureWeightConfig, features: Record<string, number>): number {
    const score = 
  ...
  private calculateOptimizationConfidence(
    ...
    const importanceSpread = Math.max(...featureImportance.map(fi => fi.importance)) - 
  ...
  private generateOptimizationRecommendations(
    ...
    const lowImportanceFeatures = featureImportance.filter(fi => fi.importance < 0.1);
    if (lowImportanceFeatures.length > 0) {
      recommendations.push(`Consider reducing weight for ${lowImportanceFeatures.map(f => f.feature).join(', ')}`);
    }
  ...
  startABTest(
    ...
    console.log(`   Traffic split: ${(trafficSplit * 100).toFixed(1)}% test, ${((1 - trafficSplit) * 100).toFixed(1)}% control`);
  ...
  recordABTestResult(
    ...
    const results = group === 'test' ? test.results.testGroup : test.results.controlGroup;
    results.sessions++;
    if (blocked) results.blocked++;
    
    // Update accuracy (simplified)
    results.accuracy = (results.accuracy * (results.sessions - 1) + (accurate ? 1 : 0)) / results.sessions;
  }
  ...
  completeABTest(testId: string): FeatureWeightConfig | null {
    ...
    console.log(`   Control group accuracy: ${(controlAccuracy * 100).toFixed(2)}%`);
    
    // Apply winning weights
    if (testAccuracy > controlAccuracy) {
      console.log(`   ✅ Test group wins! Applying new weights.`);
      this.updateWeights(test.testWeights, 'ab_test', 'A/B test winner applied');
...
export type { FeatureWeightConfig, WeightOptimizationResult, FeatureImportance, WeightValidationResult };
