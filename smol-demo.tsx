...
console.log(`- Large dataset: ${memoryIntensiveData.largeDataset.length} items`);
console.log(`- ML models: ${memoryIntensiveData.fraudDetectionModels.ensemble.models.length} models`);
console.log(`- Network metrics: ${memoryIntensiveData.networkMetrics.connections.length} connections`);
...
console.log("\n🔄 Processing data...");
const processedData = {
  filteredModels: memoryIntensiveData.fraudDetectionModels.ensemble.models
    .filter(model => model.accuracy > 0.9),
  avgLatency: memoryIntensiveData.networkMetrics.connections
    .reduce((sum, conn) => sum + conn.latency, 0) / memoryIntensiveData.networkMetrics.connections.length,
  highRiskItems: memoryIntensiveData.largeDataset
    .filter(item => item.metadata.random > 0.95)
};

console.log(`✅ Processed ${processedData.filteredModels.length} high-accuracy models`);
console.log(`✅ Average network latency: ${processedData.avgLatency.toFixed(2)}ms`);
