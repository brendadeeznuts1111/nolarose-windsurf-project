 ...
 */
export function initializeWindSurfShortcuts(): ShortcutRegistry {
...
/**
 * Get shortcut registry instance (singleton pattern)
 */
 ...
 */
export function registerWindSurfKeyboardHandlers(registry: ShortcutRegistry): void {
  if (typeof window === 'undefined') return;

  // Helper function to call API endpoint
  async function callActionAPI(endpoint: string, method: string = 'POST', body?: any) {
    try {
  ...
  // Dashboard refresh
  registry.on('shortcut:triggered', async (event) => {
 ...
 */
export function setupBrowserKeyboardHandlers(registry: ShortcutRegistry): void {
  if (typeof window === 'undefined') return;
