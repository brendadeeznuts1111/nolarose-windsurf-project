...
class RealTimeFraudDetector {
  ...
  private async runProcessors(event: StreamEvent): Promise<Array<{
    processorId: string;
    score: number;
    factors: string[];
    confidence: number;
  ...
  private async runProcessor(processorId: string, event: StreamEvent): Promise<{
    processorId: string;
    score: number;
    factors: string[];
    confidence: number;
  ...
  private analyzeTransactionPatterns(event: StreamEvent): {
    ...
    // Frequency analysis
    const recentTransactions = this.getRecentEvents(event.userId, 'transaction', 300000); // 5 minutes
  ...
  private checkVelocityPatterns(event: StreamEvent): {
    ...
    for (const period of periods) {
      const recentEvents = this.getRecentEvents(event.userId, event.type, period.ms);
      if (recentEvents.length > period.threshold) {
        score += 0.3;
        factors.push(`High velocity: ${recentEvents.length} events in ${period.name}`);
      }
    }

    // Check for rapid successive events
    const veryRecentEvents = this.getRecentEvents(event.userId, event.type, 10000); // 10 seconds
  ...
  private checkGeolocationAnomalies(event: StreamEvent): {
    ...
    if (lastLocation && this.isImpossibleTravel(lastLocation, location, event.timestamp)) {
      score += 0.6;
  ...
  private aggregateProcessorResults(event: StreamEvent, processorResults: Array<{
    processorId: string;
    score: number;
    factors: string[];
    confidence: number;
  ...
  private async handleFraudSignal(signal: FraudSignal): Promise<void> {
    ...
    // Log signal
    console.log(`🚨 FRAUD DETECTED: ${signal.riskLevel.toUpperCase()} (Score: ${signal.score.toFixed(3)})`);
  ...
  private async triggerImmediateAction(signal: FraudSignal): Promise<void> {
    ...
    console.log(`⚡ Immediate action triggered for ${signal.eventId} (${responseTime}ms)`);
  }

  // Helper methods for data retrieval
  private getRecentEvents(userId: string, eventType: string, timeMs: number): StreamEvent[] {
  ...
  private isImpossibleTravel(lastLocation: any, currentLocation: any, currentTime: number): boolean {
  ...
  private updateProcessorMetrics(eventType: string, processingTime: number, success: boolean): void {
    // Update relevant processor metrics
    for (const processor of this.processors.values()) {
      processor.eventsProcessed++;
      processor.averageLatency = (processor.averageLatency + processingTime) / 2;
  ...
  private updateProcessingMetrics(eventCount: number, processingTime: number): void {
  ...
  private updateMetrics(): void {
    ...
    this.metrics.falsePositiveRate = totalSignals > 0 ? 
      (totalSignals - confirmedFraud) / totalSignals : 0;
  }
  ...
  public getStreamMetrics(): StreamMetrics & {
    ...
    return {
      ...this.metrics,
      activeProcessors: Array.from(this.processors.values()).filter(p => p.isActive).length,
...
export { RealTimeFraudDetector, type StreamEvent, type FraudSignal, type StreamProcessor, type StreamMetrics };
