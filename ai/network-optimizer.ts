...
export class NetworkOptimizer {
  ...
  async initialize(): Promise<void> {
    ...
    process.env.BUN_CONFIG_MAX_HTTP_REQUESTS = this.config.maxSimultaneousConnections.toString();
    
    // Preconnect to all configured hosts
    await this.preconnectToHosts();
    
    console.log(`✅ Network optimizer initialized with ${this.config.preconnectHosts.length} preconnected hosts`);
  }
   ...
   */
  async batchFetch(urls: string[], options: RequestInit = {}): Promise<Response[]> {
    const batchSize = Math.min(urls.length, this.config.maxSimultaneousConnections);
    const results: Response[] = [];
    
    for (let i = 0; i < urls.length; i += batchSize) {
      const batch = urls.slice(i, i + batchSize);
      const batchPromises = batch.map(url => this.optimizedFetch(url, options));
 ...
 */
export async function batchFetch(urls: string[], options?: RequestInit): Promise<Response[]> {
