...
import { FraudSession } from "../ai/anomaly-predict";
...
const COLOR_SCHEMES = {
  ...
  plasma: ['#0d0887', '#6e00a8', '#8c0aa5', '#a8196d', '#c12b42', '#d5433d', '#e66236', '#f89441', '#fce026'],
  inferno: ['#000004', '#1b0c41', '#4a0c6b', '#781c6d', '#b5275f', '#e45936', '#f98e09', '#f8c932', '#fcffa4'],
  magma: ['#000004', '#180f3d', '#440f76', '#721f81', '#9e2f7f', '#cd4071', '#f1605d', '#fd9668', '#fca50a', '#f0f921'],
...
export class RiskHeatmap {
  ...
  addDataPoint(session: FraudSession, metadata?: HeatmapDataPoint['metadata']): void {
    ...
    console.log(`🗺️ Heatmap: Added point for ${session.merchantId} | Score: ${session.score.toFixed(3)} | Risk: ${session.riskLevel}`);
  }
  ...
  private drawDataPoints(): void {
    ...
    gridCells.forEach((points, key) => {
      ...
      const gradient = this.ctx.createRadialGradient(
        x + this.config.cellSize / 2, y + this.config.cellSize / 2, 0,
        x + this.config.cellSize / 2, y + this.config.cellSize / 2, this.config.cellSize / 2
      );
  ...
  private drawLabels(): void {
    ...
    // Merchant axis (X)
    this.ctx.fillText('Merchant Distribution', this.config.width / 2 - 50, this.config.height - 10);
    
    // Statistics
    const stats = this.calculateStatistics();
    this.ctx.fillText(`Active: ${stats.totalPoints} | Avg Risk: ${(stats.avgRiskScore * 100).toFixed(1)}%`, 10, 20);
  }
  ...
  private drawLegend(): void {
    ...
    // Color gradient
    const gradient = this.ctx.createLinearGradient(legendX + 10, legendY + 30, legendX + 10, legendY + 130);
    const colors = COLOR_SCHEMES[this.config.colorScheme];
  ...
  generate3DData(): Heatmap3DData {
    ...
    for (let x = 0; x < this.config.width; x += gridSize) {
      for (let y = 0; y < this.config.height; y += gridSize) {
        const nearbyPoints = this.dataPoints.filter(p => 
          Math.abs(p.x - x) < gridSize && Math.abs(p.y - y) < gridSize
        );
        
        if (nearbyPoints.length > 0) {
          const avgValue = nearbyPoints.reduce((sum, p) => sum + p.value, 0) / nearbyPoints.length;
          grid.push({
            x,
            y,
            value: avgValue,
  ...
  private calculateStatistics() {
    ...
    const avgRiskScore = totalPoints > 0 ? 
      this.dataPoints.reduce((sum, p) => sum + p.value, 0) / totalPoints : 0;
    const maxRiskScore = totalPoints > 0 ? 
      Math.max(...this.dataPoints.map(p => p.value)) : 0;
    
    const riskDistribution = this.dataPoints.reduce((dist, point) => {
  ...
  private detectHotspots(): Array<{
    ...
    for (let i = 0; i < this.dataPoints.length; i++) {
      ...
      if (cluster.length > 2) {
        ...
        const intensity = cluster.reduce((sum, p) => sum + p.value, 0) / cluster.length;
...
export function createRiskHeatmap(canvasId: string, config?: Partial<HeatmapConfig>): RiskHeatmap {
...
export function generateHeatmapReport(heatmap: RiskHeatmap): {
  ...
  const summary = `Risk heatmap analysis shows ${stats.totalPoints} active data points with an average risk score of ${(stats.avgRiskScore * 100).toFixed(1)}%. Current risk distribution: ${Object.entries(stats.riskDistribution).map(([level, count]) => `${level} (${count})`).join(', ')}.`;
  
  const recommendations: string[] = [];
  
  if (stats.avgRiskScore > 0.7) {
    recommendations.push('High overall risk levels detected - consider tightening thresholds');
  }
  
  if (stats.hotspots.length > 0) {
    recommendations.push(`${stats.hotspots.length} risk hotspots identified - investigate clustered fraud patterns`);
  }
  
  if (stats.riskDistribution.critical > stats.totalPoints * 0.1) {
    recommendations.push('Critical risk events above 10% - immediate security review recommended');
  }
